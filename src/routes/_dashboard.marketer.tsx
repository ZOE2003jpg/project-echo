  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Read-only view across every application stage. Search finds anyone on record, on any date."
      />
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
            <div>
              <CardTitle className="section-title flex items-center gap-2 text-base">
                ...
                {tab === "pending" ? "Active applications" : "Completed applications"}
              </CardTitle>
              <CardDescription className="tabular-nums">
                {apps.length} application{apps.length !== 1 ? "s" : ""}{" "}
                {tab === "pending" ? "in the pipeline (read-only)" : "completed"}
        ...
        <CardContent className="p-0">
          {failed ? (
            ...
            query ? (
              ...
              <EmptyState
                title={tab === "pending" ? "No active applications" : "No completed applications yet"}
                body={
                  tab === "pending"
                    ? "Nothing is currently moving through the pipeline. New applications appear here automatically."
                    : "Completed applications will be listed here once they finish the process."
                }
              />
            )
          ) : (
            <DataTable columns={columns} rows={apps} getRowKey={(a) => String(a.id)} caption="Marketer pipeline" />
          )}
        </CardContent>
      </Card>
    </div>
